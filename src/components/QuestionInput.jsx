    ...
    return (
        <div className='px-6 py-7 bg-white duration-300 ease-linear dark:bg-boxdark rounded-lg' >
            <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => handleDeleteClick(Question.id, Question.questionNumber)} className="hover:text-primary">
                            <svg
            ...
            <input
            type="text"
            placeholder='Opsi Benar'
            onChange={e => Question.setAnswer(e.target.value)}
            className="mb-4 w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
            />
            ...
            </label>
            {
                Question.options.map((option,i) => {
                    return (
                        ...
                        key={i+2}
                        type="text"
                        onChange={e => Question.setOption(i, e.target.value)}
                        placeholder={`Opsi ${i+2}`}
                        className="mb-4 w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                        />
                    )
                })
            ...
            <Select
            onChange={e => Question.setLevelId(e.target.value)}
            values={levels.map(level => level.id)}
            options={levels.map(level => `${level.name} - ${level.reward_xp} XP`)}
            />
        </div>
    )
}
